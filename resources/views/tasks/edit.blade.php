...
@section('content')
    <h1 class="text-3xl font-bold text-[#FFDBB6] mb-6">Edit Tugas</h1>

    <form action="{{ route('tasks.update', $task->id) }}" method="POST" class="space-y-6 max-w-lg">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="description" rows="4"
                class="w-full rounded border border-[#FFDBB6] bg-[#1A2A4F] text-[#FFF2EF] p-2 focus:outline-none focus:ring-2 focus:ring-[#F7A5A5]">{{ $task->description }}</textarea>
        </div>
